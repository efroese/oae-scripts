...
#
# List the viewers of the -ta group for a world.
#
...
for id in `cat $ALL_IDS`; do
	...
	curl -s -o $world_info_file -uadmin:$OAE_PASSWORD ${OAE_URL}system/userManager/group/$id-ta.tidy.json

	echo -n "$id : "

	egrep -o 'viewers":\[.*\],' $world_info_file 
